...
sleep 5  # Ждем полного запуска базы данных
if docker-compose exec -T english-bot python database/load_topics.py 2>/dev/null; then
    echo "✅ База знаний успешно загружена"
else
    echo "⚠️  База знаний может быть уже загружена или требует ручной загрузки"
    echo "💡 Для ручной загрузки выполните: docker-compose exec english-bot python database/load_topics.py"
fi
